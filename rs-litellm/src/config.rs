...
/// Supports `os.environ/VAR_NAME` syntax (like LiteLLM Python) and `${VAR_NAME}`.
...
impl Config {
    pub fn load(path: &Path) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        for entry in &mut config.model_list {
            if let Some(ref key) = entry.litellm_params.api_key {
                entry.litellm_params.api_key = Some(resolve_env_value(key));
            }
            if let Some(ref base) = entry.litellm_params.api_base {
                entry.litellm_params.api_base = Some(resolve_env_value(base));
            }
            if let Some(ref version) = entry.litellm_params.api_version {
                entry.litellm_params.api_version = Some(resolve_env_value(version));
            }
        }
...
mod tests {
    ...
    fn test_parse_provider_model() {
        ...
        assert_eq!(parse_provider_model("gpt-3.5-turbo"), ("openai", "gpt-3.5-turbo"));
    }
